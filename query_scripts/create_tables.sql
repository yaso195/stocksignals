...
CREATE TABLE IF NOT EXISTS orders (id SERIAL UNIQUE, signal_id INT REFERENCES signals(id),  order_time bigint, type TEXT NOT NULL CHECK (code <> ''), code TEXT NOT NULL CHECK (code <> ''), name TEXT NOT NULL CHECK (code <> ''), num_shares INT CONSTRAINT non_negative_num_shares CHECK (num_shares >= 0), price DECIMAL(7,2), profit DECIMAL(7,2));

CREATE TABLE IF NOT EXISTS stats (id INT REFERENCES signals(id), deposits DECIMAL(7,2), withdrawals DECIMAL(7,2), funds DECIMAL(7,2),  num_tades INT CONSTRAINT non_negative_num_trades CHECK (num_trades >= 0), price DECIMAL(7,2), profit DECIMAL(7,2));
